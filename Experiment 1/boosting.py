...
class Boosting:
    ...
    def fit(self, X_train, y_train):
        ...
        Evaluation['target'] = Y.copy()
        Evaluation['weights'] = 1/len(X_train) # Set the initial weights w = 1/N
        ...
        for t in range(self.T):
            ...
            Tree_model = DecisionTreeClassifier(criterion="entropy",max_depth=5) # Mind the deth one --> Decision Stump
            ...
            print('{0}:'.format(t+1),accuracy*100,'%')
    ...
    def get_fold_result_test(self, X_all, y_all, k):
        ...
        for train_index, test_index in kf.split(X_all, y_all):
            ...
            print('Test accuracy: ',self.accuracy[-1]*100,'%')
            self.accuracy = []
            print('-------------------------------')


    def get_fold_result(self, X_all, y_all, k):
        for t in range(self.T):
            print("######## BOOST {0} ##########".format(t+1))
            scores = []
            predictions = []
            ...
            for train_index, test_index in kf.split(X_all, y_all):
                ...
                # prep test for this fold
                Evaluation = pd.DataFrame()
                Evaluation['target'] = y_train.copy()
                Evaluation['weights'] = 1/len(X_train)
                

                Tree_model = DecisionTreeClassifier(criterion="entropy",max_depth=1) # Mind the deth one --> Decision Stump

                # fit with train for this fold
                model = Tree_model.fit(X_train,y_train,sample_weight=np.array(Evaluation['weights'])) 
            ...
            sum_predictions = [0] * len(predictions[0])
            ...
            for pred in predictions:
                for i in range(size):
                    x=sum_predictions[i]
                    y=pred[i]
                    sum_predictions[i] = x+y
                
            voted_predictions = [round(x/k) for x in sum_predictions]
            
            if len(voted_predictions) == 240:
                voted_predictions += [0]
                
            # Add values to the Evaluation DataFrame
            Evaluation['predictions'] = voted_predictions
            Evaluation['evaluation'] = np.where(Evaluation['predictions'] == Evaluation['target'],1,0)
            Evaluation['misclassified'] = np.where(Evaluation['predictions'] != Evaluation['target'],1,0)
